...
import static net.zip4j.util.BitUtils.isBitSet;
import static net.zip4j.util.BitUtils.setBit;
...
public class FileUtils {
  ...
  public static List<File> getFilesInDirectoryRecursive(File path, ZipParameters zipParameters)
    ...
    for (File file : filesAndDirs) {
      ...
      // not a symlink.
      if ((isSymLink && !ZipParameters.SymbolicLinkAction.INCLUDE_LINK_ONLY.equals(symbolicLinkAction))
      ...
      if (Zip4jUtil.isStringNotNullAndNotEmpty(zipParameters.getDefaultFolderPath())) {
        ...
        if (isSymbolicLink(fileToAdd)) {
          ...
          if (!fileCanonicalPath.startsWith(rootFolderFileRef)) {
            tmpFileName = fileToAdd.getCanonicalFile().getParentFile().getName() + InternalZipConstants.FILE_SEPARATOR + fileToAdd.getCanonicalFile().getName();
          } else {
        ...
        if (tmpFile.isDirectory()) {
          tmpFileName = tmpFileName.replaceAll("\\\\", InternalZipConstants.ZIP_FILE_SEPARATOR);
          ...
          bkFileName = bkFileName.replaceAll("\\\\", InternalZipConstants.ZIP_FILE_SEPARATOR);
