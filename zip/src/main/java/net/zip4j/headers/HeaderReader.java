...
import net.zip4j.model.AESExtraDataRecord;
import net.zip4j.model.AbstractFileHeader;
import net.zip4j.model.CentralDirectory;
import net.zip4j.model.DataDescriptor;
import net.zip4j.model.DigitalSignature;
import net.zip4j.model.EndOfCentralDirectoryRecord;
import net.zip4j.model.ExtraDataRecord;
import net.zip4j.model.FileHeader;
import net.zip4j.model.LocalFileHeader;
import net.zip4j.model.Zip4jConfig;
import net.zip4j.model.Zip64EndOfCentralDirectoryLocator;
import net.zip4j.model.Zip64EndOfCentralDirectoryRecord;
import net.zip4j.model.Zip64ExtendedInfo;
...
import static net.zip4j.headers.HeaderUtil.decodeStringWithCharset;
import static net.zip4j.util.Zip4jUtil.readFully;
...
public class HeaderReader {
  ...
  private long locateOffsetOfEndOfCentralDirectoryByReverseSeek(RandomAccessFile randomAccessFile) throws IOException {
    long currentFilePointer = randomAccessFile.length() - InternalZipConstants.ENDHDR;
    // reverse seek for a maximum of MAX_COMMENT_SIZE bytes
    long numberOfBytesToRead = randomAccessFile.length() < InternalZipConstants.MAX_COMMENT_SIZE ? randomAccessFile.length() : InternalZipConstants.MAX_COMMENT_SIZE;
  ...
  private String readZipComment(RandomAccessFile raf, int commentLength, Charset charset) {
    ...
    try {
      ...
      return decodeStringWithCharset(commentBuf, false, charset != null ? charset : InternalZipConstants.ZIP4J_DEFAULT_CHARSET);
    } catch (IOException e) {
